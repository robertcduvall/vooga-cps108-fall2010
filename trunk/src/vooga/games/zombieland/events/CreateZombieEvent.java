...
public class CreateZombieEvent implements IEventHandler{
	...
	public CreateZombieEvent(PlayState current, Shooter shooter, IEventHandler endLevel, IEventHandler addZombies)
	{
		isUsedOnce = false;
	...
	public void createZombies() {
		...
		while(size < levelZombies()){
			addZombies.addEnemy(new Zombie("New", level, player, currentPlayState));
			size++;
		}
	...
	public int levelZombies() {
		return (int) (Zombie.zombiesPerLevel() * level* 
				(Double)Resources.getDouble("zombieLimitingFactor"));
	}
