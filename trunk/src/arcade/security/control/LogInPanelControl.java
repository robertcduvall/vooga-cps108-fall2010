...
public class LogInPanelControl implements IControl {
	...
	private void switchToForgetPasswordPage() {
		...
		RetrievePasswordPanelControl controller = new RetrievePasswordPanelControl(
				(IView) jp, model);
	}
	...
	private class SubmitEvent implements ActionListener {
		@Override
		public void actionPerformed(ActionEvent e) {
			String username = view.getUserNameUserInput().toString();
			String password = view.getPasswordUserInput().toString();
			if (!isSuccessfulLogin(username, password)) { // next step towards
				// true mvc:move
				// this to
				// controller
				...
				JOptionPane
						.showMessageDialog(view.getCurrentPanel(),
								"Your username and password combination does not match");
			...
			//JOptionPane.showMessageDialog(view.getCurrentPanel(),"privilege is"
			// +PrivilegeMap.getPrivilegeString());
	...
	// Below might not be the best way to handle switching between different
	// view, we will discuss.
	// /**
	...
	// */
	// private static JComponent getPanel(String className) throws
	// ClassNotFoundException, IllegalArgumentException, SecurityException,
