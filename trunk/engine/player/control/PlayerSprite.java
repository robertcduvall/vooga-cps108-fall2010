...
public class PlayerSprite extends GameEntitySprite {
	 ...
	 * @param stateName is the name that will be associated with the Sprite parameter (to switch to it).
	 * @param s is a Sprite that will represent this player.
	 * @param control is a class that implements the IPlayerController interface (for controlling player speed and movement).
	 */
	public PlayerSprite(String name, String stateName, Sprite s, IPlayerController control) 
	{
	 ...
	 */
	public PlayerSprite(String name, String stateName, Sprite s, IPlayerController control, int playerHealth, int playerRank) 
	{
	...
	/*
	 * Checks for any inputs from controller class and updates player's current sprite accordingly.
	 */
	public void update(long elapsedTime) 
	{
		...
		if(this.getY()+myController.deltaY() < this.getBackground().getHeight()
				&& this.getY()+myController.deltaY() > 0) {
			this.move(0,myController.deltaY());
		}
		if(this.getX()+myController.deltaX() < this.getBackground().getWidth()
				&& this.getX()+myController.deltaX() > 0) {
	...
	/**
	 * Sets rank to DEFAULT_RANK, health to DEFAULT_HEALTH,
	 * score to DEFAULT_SCORE, lives to DEFAULT_LIVES
	 */
