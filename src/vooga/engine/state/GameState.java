...
public abstract class GameState {
	...
	public boolean checkMessage() {
		if (message == null || message.length() == 0 || message.equals("still")) {
			...
			try {
				String name = message.indexOf(":") == -1 ? message : message
						.substring(0, message.indexOf(":"));
				String param = message.indexOf(":") == -1 ? null : message
						.substring(message.indexOf(":") + 1);
				Method statusAction = param == null ? this.getClass()
						.getMethod(name) : this.getClass().getMethod(name,
						String.class);
				message = null;
				return param == null ? ((Boolean) (statusAction.invoke(this)))
						.booleanValue() : ((Boolean) (statusAction.invoke(this,
						param))).booleanValue();
			} catch (Exception e) {
				System.out
	...
	public SpriteGroup getGroup(String name) {
		for (PlayField playfield : myUpdateField) {
			for (SpriteGroup sg : playfield.getGroups())
