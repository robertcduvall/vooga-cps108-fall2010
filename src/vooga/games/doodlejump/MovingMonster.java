...
public class MovingMonster extends Sprite{
	...
	public void update(long elapsedTime){
		...
		if(getX() < 0 || getX() > 532 - getWidth()){
			if(getX() < 0){
				try {
					setImage(ImageIO.read(new File("src/vooga/games/doodlejump/images/blue_monster_right.png")));
				} catch (IOException e) {
					e.printStackTrace();
				}
			}
				...
				try {
					setImage(ImageIO.read(new File("src/vooga/games/doodlejump/images/blue_monster_left.png")));
				} catch (IOException e) {
					e.printStackTrace();
				}
			}
